...
export interface IRaceSocketPayload extends Partial<ITimeTracking> {
  _id?: string
  status?: string
  stampId?: string
  startTime?: Date
...
export async function setTracking(data: IRaceSocketPayload) {
  const now = new Date()
...
export async function assignTracking(data: IRaceSocketPayload) {
  if (data.bib || data.participantId) {
    try {
      const timeTracking = await setTracking(data)
      await deleteOneUnassignedTimeTracking({
        raceId: data.raceId,
        segmentId: data.segmentId,
...
export async function resetTracking(data: IRaceSocketPayload) {
  if (data.bib || data.participantId) {
    try {
...
export const deleteOneUnassignedTimeTracking = async (
  ...
  try {
    ...
    throw new StatusError('Failed to delete unassigned track time', 500, error)
  }
}
// reset
...
export const createUnassignedTimeTracking = async (
  data: IRaceSocketPayload
) => {
  try {
    // Step 1: Create new TimeTracking document
...
export const assignTimeTracking = async (data: IRaceSocketPayload) => {
  try {
    // Step 1: Create new TimeTracking document
